"""Pure-Python oracle for phase 2 (PnL accounting) on analytic cases.

This is the *executable specification* that the Rust loop must reproduce (parity).
"""
...
def test_accumulate_long_one_unit_zero_cost_tracks_price_returns():
    ...
    positions = np.array([1.0, 1.0, 1.0], dtype=np.float64)  # always long 1
    returns, n_trades = accumulate(positions, prices, fees_bps=0.0, slippage_bps=0.0)
    # t0: entry, prev_pos=0, no market move -> 0
    assert returns[0] == 0.0
    # t1: prev_pos=1, market +10 % -> +0.10
    assert np.isclose(returns[1], 0.10)
    # t2: prev_pos=1, market 99/110-1
    assert np.isclose(returns[2], 99.0 / 110.0 - 1.0)
    assert n_trades == 1  # a single entry at t0


def test_accumulate_charges_entry_and_exit_costs():
    prices = np.array([100.0, 100.0], dtype=np.float64)  # flat market
    positions = np.array([1.0, 0.0], dtype=np.float64)  # enters then exits
    returns, n_trades = accumulate(positions, prices, fees_bps=10.0, slippage_bps=0.0)
    # entry +1 then exit -1, 10 bps on |delta|=1 -> -0.001 each
...
def test_accumulate_flat_position_yields_zero():
    ...
    positions = np.zeros(3, dtype=np.float64)  # never invested
